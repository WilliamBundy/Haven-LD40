...
void addSegment(TextureAtlas* atlas, TextureSegment* s)
{
...
void addFont(
		...
		MemoryArena* arena)
{
	void* fontFile = wplLoadFont(window, filename, arena);
	void* fontCtx = wplCreateFontContext(fontFile, arena);
	font->scale = wplGetFontScale(fontCtx, (i64)heightInPixels);
	for(isize i = 0; i < rangesCount; ++i) {
		isize start = ranges[i*2];
		isize end = ranges[i*2+1];
		//this will just fail outside of 256... but, whatever
		wplPopulateKerning(fontCtx, font, start, end);
		for(isize ci = start; ci < end; ++ci) {
			isize index = wplGetGlyphIndexFromCodepoint(fontCtx, ci);
			wplGlyph* g = glyphs + ci;
			wplPopulateGlyph(fontCtx, index, g, font->scale);
			wplRenderGlyph(fontCtx, g, arena);
			TextureSegment s;
			s.name[0] = (u8)(ci & 0xFF);
			s.hash = 0;
			s.region.w = g->sw;
